...
const categories = ['Tous', 'SEO', 'Stratégie', 'Site Web', 'Google Ads', 'E-commerce', 'Développement', 'Automatisation', 'IA']
...
function filterHref(category: string, city: string, page = 1) {
...
export function BlogArticles({ articles, category: requestedCategory = 'Tous', city: requestedCity = 'Toutes les villes', page: requestedPage = 1 }: { articles: BlogCardArticle[]; category?: string; city?: string; page?: number }) {
  ...
  return <section id="articles" className="blog-list" aria-label="Nos articles et conseils">
    <div className="blog-container">
      ...
      <div aria-busy={isPending}>
        {filtered.length ? <div className="blog-articles-grid">{filtered.slice((page - 1) * 12, page * 12).map(article => <article className="blog-article-card" key={article.slug}>
      ...
      {pageCount > 1 && <nav className="editorial-pagination" aria-label="Pagination des articles">{page > 1 && <Link href={filterHref(category, city, page - 1)}>Articles précédents</Link>}<span>Page {page} sur {pageCount}</span>{page < pageCount && <Link href={filterHref(category, city, page + 1)}>Articles suivants</Link>}</nav>}
    </div>
  </section>
}
