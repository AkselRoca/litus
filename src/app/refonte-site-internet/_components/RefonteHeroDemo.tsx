  ...
  return <div ref={root} className="refonte-hero-demo" data-hydrated={hydrated} data-view={view} role="group" aria-label="Démonstration d’une refonte et de sa migration">
    <div className="refonte-demo-stage"><div className="refonte-demo-frame">
      ...
      <div className="refonte-demo-panel" role="tabpanel" id={`${id}-panel`} aria-labelledby={`${id}-tab-${view}`} tabIndex={0} onFocus={() => setPlaying(false)}>
        {view < 2 ? <div className="refonte-demo-site" data-new={view === 1} key={view}><div className="refonte-demo-browser"><LayoutTemplate aria-hidden="true" /><span>Votre entreprise</span><small>{view === 0 ? 'Présentation · Prestations · Contact' : 'Expertises · Réalisations · Contact'}</small></div><div className="refonte-demo-site-body"><span>{view === 0 ? 'VOTRE SAVOIR-FAIRE' : 'UNE OFFRE QUI SE COMPREND'}</span><h3>{view === 0 ? 'Une entreprise, de nombreux services.' : 'Le bon service. Une prochaine étape claire.'}</h3><p>{view === 0 ? 'Des contenus utiles, à organiser autour des besoins de vos visiteurs.' : 'Votre expertise, des réponses concrètes et un accès direct au contact.'}</p><div className="refonte-demo-page-items"><span><FileText aria-hidden="true" />{view === 0 ? 'Présentation' : 'Vos besoins'}</span><span><Link2 aria-hidden="true" />{view === 0 ? 'Nos prestations' : 'Nos réponses'}</span></div><span className="refonte-demo-site-cta">{view === 0 ? 'En savoir plus' : 'Parlons de votre projet'}<ArrowRight aria-hidden="true" /></span></div></div> : <div className="refonte-demo-migration"><span className="refonte-demo-migration-icon"><Link2 aria-hidden="true" /></span><h3>Les bonnes pages,<br />aux bonnes adresses.</h3><p>Un plan de redirection préparé avant la bascule.</p><ol>{[['/prestations.html', '/services'], ['/societe.html', '/a-propos'], ['/nous-joindre', '/contact']].map(([from, to]) => <li key={from}><span>{from}</span><i>301 <ArrowRight aria-hidden="true" /></i><strong>{to}</strong>{checks && <Check aria-hidden="true" />}</li>)}</ol><button type="button" onClick={() => { setVerified(true); setPlaying(false) }}>{checks ? <CheckCheck aria-hidden="true" /> : <ShieldCheck aria-hidden="true" />}{checks ? 'Parcours d’exemple vérifiés' : 'Vérifier les parcours'}</button></div>}
      </div>
