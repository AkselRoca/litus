...
import connectedTools from './connected-tools.json'
import type { ExpertiseSlug, ExpertiseTool } from './types'

export const expertiseTools: ExpertiseTool[] = [
  { slug: 'python', name: 'Python', logo: '/brands/color/python.svg', category: 'Développement backend & data', summary: 'Traiter des fichiers, relier des API et automatiser des opérations avec des erreurs et des reprises maîtrisées.' },
  ...
  { slug: 'labview', name: 'LabVIEW', logo: '/brands/color/labview.png', category: 'Ingénierie, acquisition & mesure', summary: 'Développer des interfaces opérateur, reprendre des VIs et organiser les mesures selon votre matériel et votre protocole.' },
  ...
  ...connectedTools.map(tool => ({ ...tool, slug: tool.slug as ExpertiseSlug })),
]
...
export const homeTools = expertiseTools.filter(tool => !['python', 'csharp', 'dotnet', 'labview'].includes(tool.slug))
const baseColors: Record<string, string> = { nextjs: '#171717', react: '#087ea4', typescript: '#3178c6', tailwind: '#0891b2', framer: '#0055ff', vercel: '#171717', stripe: '#635bff', shopify: '#537e26', wordpress: '#21759b', python: '#3776ab', csharp: '#8034b8', dotnet: '#512bd4', labview: '#947400' }
export function expertiseStyle(slug: ExpertiseSlug): CSSProperties {
  const color = connectedTools.find(tool => tool.slug === slug)?.color || baseColors[slug] || '#193447'
  return { '--tool-color': color, '--tool-tint': color + '0c' } as CSSProperties
}
