...
export function expertiseMetadata(title: string, description: string, path: string): Metadata {
  return {
    ...
    openGraph: { title, description, url: `https://litus.fr${path}`, type: 'website', siteName: 'Litus', locale: 'fr_FR', images: [{ url: '/litus-og-social.png', width: 1200, height: 630, alt: 'Litus : acquisition et infrastructure digitale' }] },
...
export function expertiseSchema(page?: ExpertisePage) {
  const url = `https://litus.fr${page ? expertisePath(page.slug) : '/expertise'}`
