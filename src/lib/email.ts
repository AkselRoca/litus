import 'server-only'
import type { ContactSubmissionData as ContactFormData } from './validations/contact'
import type { ContactConfig } from './contact/config'

export const CONTACT_RECIPIENT = 'litusagency@gmail.com'
export const serviceLabels: Record<NonNullable<ContactFormData['service']>, string> = {
...
export function contactEmail(data: ContactFormData, from: string, technical?: ContactTechnicalInfo) {
  const serviceLabel = data.service ? serviceLabels[data.service] : ''
  const budgetLabel = data.budget ? budgetLabels[data.budget] : ''
  const fields = [ ['Nom', data.nom], ['Email', data.email], ['Téléphone', data.telephone || ''],
    ['Entreprise / Site web', data.entreprise || ''], ['Service souhaité', serviceLabel],
    ['Budget estimé', budgetLabel], ['Description du projet', data.message] ].filter(([, value]) => value)
...
- Projet : ${serviceLabel || 'Non renseigné'}
- Budget : ${budgetLabel || 'Non renseigné'}
...
export function contactConfirmationEmail(data: ContactFormData, from: string) {
  ...
  const service = escapeHtml(serviceLabel)
  const summaryFields = [
    ['Projet', serviceLabel],
