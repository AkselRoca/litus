...
export function ExpertiseArticle({ page }: { page: ExpertisePage }) {
  ...
  return <div className={`business-service expertise-page expertise-${page.slug}`}>
    ...
    <div className="business-content ex-content">
      ...
      {page.sections.map((section, index) => <Fragment key={section.id}><section id={section.id} key={section.id} aria-labelledby={`${section.id}-title`} className={`business-section ex-section ex-section-${section.kind}${section.kind === 'workflow' ? ' business-section-soft' : index % 3 === 1 && section.kind !== 'issues' ? ' business-section-warm' : ''}`}><div className="business-container"><SectionContent section={section} /></div></section>{index === 0 && <ExpertiseGallery slug={page.slug} index={0} />}{index === 2 && <ExpertiseGallery slug={page.slug} index={1} />}</Fragment>)}
      ...
      <section className="business-section ex-sources-section" aria-label="Références techniques"><div className="business-container"><details className="ex-sources"><summary>Références techniques consultées<ChevronDown size={16} aria-hidden="true" /></summary><p>Documentation consultée le 12 septembre 2026. Les versions, forfaits et compatibilités sont revérifiés au cadrage de chaque intervention. Les marques citées appartiennent à leurs titulaires ; leur présence ne revendique aucun partenariat.</p>{page.slug === 'framer' && <p className="ex-brand-notice">Ce contenu et les services Litus sont indépendants de Framer B.V., sans affiliation, approbation ni parrainage de sa part. <a href="https://www.framer.com/" target="_blank" rel="noopener noreferrer">Découvrir la plateforme Framer</a>.</p>}{page.slug === 'shopify' && <p className="ex-brand-notice"><a href="https://www.shopify.com/" target="_blank" rel="noopener noreferrer">Découvrir la plateforme Shopify</a>. Aucune certification ni affiliation n’est revendiquée.</p>}<ul>{page.sources.map(source => <li key={source.href}><a href={source.href} target="_blank" rel="noopener noreferrer">{source.label}<ArrowUpRight size={14} aria-hidden="true" /></a></li>)}</ul></details></div></section>
