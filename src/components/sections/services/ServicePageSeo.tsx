...
export function serviceMetadata({ path, title, description, name, image }: ServiceSeo): Metadata {
  return {
    ...
    openGraph: { title, description, url: `https://www.litus.fr${path}`, type: 'website', siteName: 'Litus', locale: 'fr_FR', images: [{ url: image, width: 1400, height: 933, alt: name }] },
...
export function ServicePageSeo({ path, name, description, questions }: { path: string; name: string; description: string; questions: readonly ServiceQuestion[] }) {
  const url = `https://www.litus.fr${path}`
  const data = { '@context': 'https://schema.org', '@graph': [
    { '@type': 'Service', '@id': `${url}#service`, url, name, description, provider: { '@type': 'Organization', name: 'Litus', url: 'https://www.litus.fr' } },
    { '@type': 'BreadcrumbList', itemListElement: [{ '@type': 'ListItem', position: 1, name: 'Accueil', item: 'https://www.litus.fr' }, { '@type': 'ListItem', position: 2, name, item: url }] },
